...
def render_walls(scene, surface, textures):
    ...
    for x, ray in scene.camera.rays(surface.get_width()):
...
def render_sprites(scene, surface, z_buf, sprites):
    pos_x, pos_y = scene.camera.pos
    plane_x, plane_y = scene.camera.screen
    dir_x, dir_y = scene.camera.dir
    scene.sprites.sort(
        reverse=True,
