...
RayHitInfo = namedtuple("RayHitInfo", ["hit", "distance", "horizontal", "wall_x_rat"])
...
def _get_ray_hit_info(pos, ray, world):
    ...
    x_tile_dist = math.sqrt(1 + (ray_y * ray_y) / ((ray_x * ray_x) if ray_x != 0 else float('nan')))
    y_tile_dist = math.sqrt(1 + (ray_x * ray_x) / ((ray_y * ray_y) if ray_y != 0 else float('nan'))) 
    
    x_step = 1 if ray_x > 0 else -1 
    x_dist_moved = x_tile_dist * ((1 + tile_x - pos_x) if ray_x > 0 else (pos_x - tile_x))

    y_step = 1 if ray_y > 0 else -1 
    y_dist_moved = y_tile_dist * ((1 + tile_y - pos_y) if ray_y > 0 else (pos_y - tile_y)) 
...
class Scene:
    ...
    def render(self, surface, textures):
        for x, ray in self.camera.rays(surface.get_width()):
            ...
            column = _get_column(textures[ray_info.hit - 1], ray_info.wall_x_rat, height)
